...
| Setting | Value |
| --- | --- |
| Tag Prefix | `KoreForge.Metrics/v` |
| Auto Increment | `minor` |
| Default Pre-release | `alpha.0` |

Example release tag: `KoreForge.Metrics/v1.4.0`
...
```powershell
git describe --tags --match "KoreForge.Metrics/v*"
```
...
| Scenario | Command |
| --- | --- |
| Check current version | `git describe --tags --match "KoreForge.Metrics/v*"` |
...
```xml
<PropertyGroup>
  <MinVerTagPrefix>KoreForge.Metrics/v</MinVerTagPrefix>
